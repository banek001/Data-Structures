...
void BST::DeleteVertex(el_t E)
{
  ...
  { cout << "... deleting the root with just one child" << endl; 
    ...
    { 
      V = Root->Left; // change the root to the left child
     
      delete Root; //   making sure the old root has been deleted
      Root = V;
      V->Up = NULL;// and the new root's UP is NULL 
      return;// and return
    }
   
    else{ // and it has only the right subtree
     
      V = Root->Right; // change the root to the right child
 
      delete Root;     //    making sure the old root has been deleted and
      Root = V;
      V->Up = NULL;// the new root's UP is NULL 
      return;// and return
...
void BST::remove(Vertex *V, Vertex *P)
{
  ...
  if((V->Left == NULL) && (V->Right == NULL))
	...
	if(P->Left == V)	//  if V is a left child of P
	{
          ...
          delete V; //  delete V and
	  P->Left = NULL;// also make Parent's left NULL 
	  ...
	  { 
	    ...
	    delete V; // delete V and
	    P->Right = NULL;// also make Parent's right NULL 
