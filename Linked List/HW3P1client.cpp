...
{ 
  ...
  catch(llist::OutOfRange){cout<<"OUT OF Range"<<endl;} // do not exit
  ...
  try {L.deleteIth(5,x); }
  catch(llist::OutOfRange){cerr<<"OUT OF Range"<<endl;} // do not exit
  ...
  try {L.deleteFront(x);}
  catch (llist::Underflow){cerr<<"You caused UnderFlow"<<endl;}
  ...
  try {L.deleteRear(x); }
  catch (llist::Underflow){cerr<<"You cause Underflow"<<endl;}
