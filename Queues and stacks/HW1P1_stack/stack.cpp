...
{  cout << "STACK:------------------------" << endl;
   ...
   if (isEmpty()) cout << "[ empty ]" << endl;// if empty printing [empty]
   else for (int i = top; i>= 0 ; i--)//else printing whatever is left in the stack
      { cout << el[i] << endl; }
   cout << "------------------------------" << endl;
}
...
void stack::clearIt()
{
   el_t temp;  // "temp" ** comment a local variable here
   // do not do top = -1, use a loop
    while(!isEmpty())
      {
	pop(temp);//popping into temp variable
      }  
}
